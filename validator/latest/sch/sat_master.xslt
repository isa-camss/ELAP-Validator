...
<xsl:function as="xs:boolean" name="local:inElementSet">
  ...
  <xsl:function as="xs:string*" name="local:abbFromPrinciple">
        <xsl:param name="element" />
        <xsl:variable name="abb" select="let $principlePURI := $element/a:properties/a:property[             @propertyDefinitionRef = string($root/a:model/a:propertyDefinitions/a:propertyDefinition[string(a:name) = 'eira:PURI']/@identifier)            ]/a:value return(                 let $satPrinciple := $satPrinciples[                     string(a:properties/a:property[                         @propertyDefinitionRef = string($satDoc/a:model/a:propertyDefinitions/a:propertyDefinition[string(a:name) = 'eira:PURI']/@identifier)                     ]/a:value) = $principlePURI                ] return (                    let $principleIdentifier := $satPrinciple/@identifier return (                       $satAbb[                             exists(                             $satDoc/a:model/a:relationships/a:relationship[                                 @target = @identifier and @source = $principleIdentifier                             ])                       ]/a:name                    )                )            )         " />
        <xsl:sequence select="string-join($abb, ', ')" />
    </xsl:function>
  <xsl:function as="xs:string*" name="local:extractAbbRelatedToPrinciple">
        <xsl:param name="element" />
        <xsl:variable name="abb" select="$inputAbb[    let $principleIdentifier := $element/@identifier return (      exists(      $root/a:model/a:relationships/a:relationship[       @target = @identifier and @source = $principleIdentifier      ])    )]/a:name         " />
        <xsl:sequence select="string-join($abb, ', ')" />
    </xsl:function>
  <xsl:function as="xs:string*" name="local:extractSbbRelatedToPrinciple">
        <xsl:param name="element" />
        <xsl:variable name="sbb" select="$inputSbb[             let $principleIdentifier := $element/@identifier return (                     exists(                     $root/a:model/a:relationships/a:relationship[                         @target = @identifier and @source = $principleIdentifier                     ])             )]/a:name         " />
  ...
  <xsl:function as="xs:boolean" name="local:findSbbRelatedToPrinciple">
        <xsl:param name="element" />
		<xsl:variable name="sbbRelatedToInput" select="$inputSbb[    $root/a:model/a:relationships/a:relationship[@source = $element/@identifier]/@target = @identifier]" />
		...
		<xsl:variable name="result" select="every $abbType in $abbRelatedToSbbTypes satisfies(     exists(      $satAbb/a:properties/a:property[       @propertyDefinitionRef = $satDoc/a:model/a:propertyDefinitions/a:propertyDefinition[a:name = 'dct:type']/@identifier and       a:value = $abbType      ]     ) or     not(exists($sbbRelatedToInput))    )" />
    	<xsl:sequence select="$result" />
    </xsl:function>
...
<svrl:text>Architecture Principle integrity validation</svrl:text>
  ...
  <xsl:param name="satAbb" select="$satDoc/a:model/a:elements/a:element[        a:properties/a:property[                     @propertyDefinitionRef = $root/a:model/a:propertyDefinitions/a:propertyDefinition[a:name = 'eira:concept']/@identifier                 ]/a:value = 'eira:ArchitectureBuildingBlock']" />

<!--PATTERN CommonCompleteness-->


	<!--RULE -->
<xsl:template match="/a:model" mode="M25" priority="1000">
...
<!--PATTERN Common-->
	...
	<!--RULE -->
<xsl:template match="/a:model/a:elements/a:element[local:isArchitecturePrinciple(.)]" mode="M26" priority="1000">
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="local:findSbbRelatedToPrinciple(.)">
          ...
          <svrl:text>[ELAP-005] Architecture principle '<xsl:text />
            <xsl:value-of select="./a:name" />
            <xsl:text />' must be modelled and related to the correct SBBs (currently are: <xsl:text />
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="local:findAbbRelatedToPrinciple(.)">
          ...
          <svrl:text>[ELAP-004] Architecture principle '<xsl:text />
            <xsl:value-of select="./a:name" />
            <xsl:text />' must be modelled and related to the correct ABB (currently are: <xsl:text />
            <xsl:value-of select="local:extractAbbRelatedToPrinciple(.)" />
            <xsl:text />): <xsl:text />
            <xsl:value-of select="local:abbFromPrinciple(.)" />
            <xsl:text />.</svrl:text>
        </svrl:failed-assert>
