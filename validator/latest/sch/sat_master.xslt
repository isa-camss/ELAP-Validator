...
<xsl:function as="xs:boolean" name="local:inElementSet">
  ...
  <xsl:function as="xs:boolean" name="local:isRealisedByInteroperabilitySpecification">
        ...
        <xsl:sequence select="exists($ancestors[             let $elementIdentifier := @identifier return (                 $root/a:model/a:relationships/a:relationship[                     @target = $elementIdentifier                     and @xsi:type = 'Realization'                     and (let $otherElementIdentifier := @source return (                         $root/a:model/a:elements/a:element[@identifier = $otherElementIdentifier and (                             exists(local:findAncestorElements(., $root/..)[local:hasStereotype(., 'Interoperability Specification')])                             or exists(local:findChildElements(., $root/..)[local:hasStereotype(., 'Interoperability Specification')])                         )]                     ))                 ]             )         ])" />
    </xsl:function>
  <xsl:function as="element()*" name="local:findAllRelatedElements">
        ...
        <xsl:variable name="foundElements" select="      $root/a:model/a:elements/a:element[         not(local:inElementSet(@identifier, $checkedElements)) and (            let $otherElementIdentifier := @identifier return (               $root/a:model/a:relationships/a:relationship[                  ($elementIdentifier = @target and $otherElementIdentifier = @source)                  or ($elementIdentifier = @source and $otherElementIdentifier = @target)               ]            )         )      ]     " />
  ...
  <xsl:function as="element()*" name="local:findAncestorElements">
        ...
        <xsl:param name="checkedElements" />
        <xsl:variable name="elementIdentifier" select="$element/@identifier" />
        <xsl:variable name="foundElements" select="      $root/a:model/a:elements/a:element[         not(local:inElementSet(@identifier, $checkedElements)) and (            let $otherElementIdentifier := @identifier return (               $root/a:model/a:relationships/a:relationship[                  ((@xsi:type = 'Realization') and ($elementIdentifier = @source and $otherElementIdentifier = @target))                     or ((@xsi:type = 'Specialization') and ($elementIdentifier = @source and $otherElementIdentifier = @target))                     or ((@xsi:type = 'Aggregation') and ($elementIdentifier = @target and $otherElementIdentifier = @source))                     or ((@xsi:type = 'Composition') and ($elementIdentifier = @target and $otherElementIdentifier = @source))               ]            )         )      ]     " />
        <xsl:variable as="element()*" name="checkedElementsWithCurrent">
            <xsl:if test="exists($checkedElements)">
                <xsl:for-each select="$checkedElements">
                    <xsl:copy-of select="." />
  ...
  <xsl:function as="element()*" name="local:findChildElements">
        ...
        <xsl:param name="checkedElements" />
        <xsl:variable name="elementIdentifier" select="$element/@identifier" />
        <xsl:variable name="foundElements" select="      $root/a:model/a:elements/a:element[         not(local:inElementSet(@identifier, $checkedElements)) and (            let $otherElementIdentifier := @identifier return (               $root/a:model/a:relationships/a:relationship[                  ((@xsi:type = 'Realization') and ($elementIdentifier = @target and $otherElementIdentifier = @source))                     or ((@xsi:type = 'Specialization') and ($elementIdentifier = @target and $otherElementIdentifier = @source))                     or ((@xsi:type = 'Aggregation') and ($elementIdentifier = @source and $otherElementIdentifier = @target))                     or ((@xsi:type = 'Composition') and ($elementIdentifier = @source and $otherElementIdentifier = @target))               ]            )         )      ]     " />
        <xsl:variable as="element()*" name="checkedElementsWithCurrent">
            <xsl:if test="exists($checkedElements)">
                <xsl:for-each select="$checkedElements">
                    <xsl:copy-of select="." />
...
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="" title="Architecture Principle integrity validation">
      ...
      <svrl:active-pattern>
        ...
        <xsl:attribute name="id">SATSpecific</xsl:attribute>
        <xsl:attribute name="name">SATSpecific</xsl:attribute>
        <xsl:apply-templates />
      </svrl:active-pattern>
